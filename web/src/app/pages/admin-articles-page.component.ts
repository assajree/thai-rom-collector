...
export class AdminArticlesPageComponent {
  ...
  protected async publish(item: Article) { this.editing=item.id; this.form={...item,tags:[...(item.tags??[])]}; await this.save('published'); }
  protected async save(status: 'draft'|'published') { if(this.busy)return; this.form.status=status; if(!this.form.title.trim()||!this.form.content.trim()||!this.form.slug.trim()){this.status.show('กรุณากรอกชื่อเรื่อง, slug และเนื้อหา','error');return;} this.busy=true; this.status.show(status==='published'?'กำลังเผยแพร่บทความ…':'กำลังบันทึกบทความ…'); try{await this.repo.save({...this.form,tags:this.form.tags},this.editing??undefined); this.status.show(status==='published'?'เผยแพร่บทความสำเร็จ':'บันทึกบทความสำเร็จ','success'); this.form=this.blank(); this.editing=null; await this.loadItems(); await this.router.navigate(['/admin/articles']);}catch(e){this.status.show(e instanceof Error?e.message:'ไม่สามารถบันทึกบทความได้','error');}finally{this.busy=false;} }
