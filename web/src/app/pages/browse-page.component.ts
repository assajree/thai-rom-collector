...
import { PatchTableComponent } from '../components/patch-table.component';

@Component({
  selector: 'app-browse-page',
  standalone: true,
  imports: [GameListControlsComponent, PatchCardListComponent, PatchTableComponent],
...
export class BrowsePageComponent {
  ...
  protected readonly sortBy = signal<'gameTitle' | 'translatedBy' | 'system'>('gameTitle');
  ...
  protected readonly sortedPatches = computed(() => this.patches().filter((patch) => {
    ...
    const field = this.sortBy(); const primary = a[field].localeCompare(b[field], 'th', { sensitivity: 'base' });
    if (primary !== 0) return primary * (this.direction() === 'asc' ? 1 : -1);
    return a.id.localeCompare(b.id);
  }));
  protected setSort(value: 'gameTitle' | 'translatedBy' | 'system'): void { this.sortBy.set(value); }
  ...
  protected setViewMode(value: 'card' | 'table'): void { this.viewMode.set(value); }
